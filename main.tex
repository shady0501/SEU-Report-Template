...
% !Mode:: "TeX:UTF-8"
\documentclass[bachelor,nocolorlinks,seutextandcolorlogo,printoneside]{seuthesis} % 本科
% noseulogo、seutextlogo、seutextandcolorlogo 三种风格可自由切换
...
\coursetitle{xx课程实验报告}
\showreporttitletrue     % 显示报告标题
\showdepartmenttrue      % 显示院系
\showmajortrue           % 显示专业
\showstudentidtrue       % 显示学号
\showstudentnametrue     % 显示学生姓名
\showadvisortrue         % 显示导师姓名
\showdurationtrue        % 显示起讫日期
\showaddresstrue         % 显示设计地点

\headerlabel{东南大学xx课程实验报告} % 页眉标签设置

\reporttitle{题目}
\major{软\quad 件\quad 工\quad 程}
\department{软\quad 件\quad 学\quad 院}
